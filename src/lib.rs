...
trait Readable<R> {
    fn read(reader: &mut R) -> Result<Self> where Self: Sized;
}
trait Writable<W> {
...
fn write_properties_none_terminated<W: Write>(writer: &mut W, properties: &Vec<Property>) -> Result<()> {
    for p in properties {
        p.write(writer)?;
    }
...
fn read_array<T, R: Read>(length: u32, reader: &mut R, f: fn(&mut R) -> Result<T>) -> Result<Vec<T>> {
...
impl MapEntry {
    fn read<R: Read>(key_type: &PropertyType, value_type: &PropertyType, reader: &mut R) -> Result<MapEntry> {
